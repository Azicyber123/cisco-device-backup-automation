...
BACKUP_DIR = BASE_DIR / "backups" / "nxos"
LOG_DIR = BASE_DIR / "logs"
LOG_FILE = LOG_DIR / "cisco_nxos_backup.log"
KEEP_DAYS = 30
...
try:
    with open(BASE_DIR / "config" / "devices.yaml", "r") as f:
        ...
        SWITCHES = [d for d in config.get("devices", []) if d.get("type") == "nxos"]
...
if not SWITCHES:
    raise Exception("❌ No NX-OS devices found in devices.yaml")
...
log.info("=" * 60)
log.info("   Cisco NX-OS Backup Tool — Started")
log.info("=" * 60)
...
send_teams(
    title="📦 Cisco NX-OS Backup Summary",
...
if failed:
    ...
    send_email(
        subject=f"⚠️ Cisco NX-OS Backup — {len(failed)} Failure(s)",
        body=f"Backup completed with errors.\n\nFailed Devices:\n{failed_list}"
    )

log.info("✅ Cisco NX-OS Backup Process Completed.")